...
public class WaxedZombifiedCutPigsteel extends Block {
    ...
    public WaxedZombifiedCutPigsteel(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(BlockState state, Level world, BlockPos blockPos, Player playerEntity, InteractionHand hand, BlockHitResult result) {
        if(playerEntity.getItemInHand(hand).getItem() instanceof AxeItem) {
            ...
            world.setBlockAndUpdate(blockPos, BlockInit.ZOMBIFIED_CUT_PIGSTEEL.get().defaultBlockState());
        }
