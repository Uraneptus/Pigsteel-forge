...
public class CutPigsteelBlock extends Block {
    ...
    public InteractionResult use(BlockState blockState, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        WAX_OFF_LIST.put(BlockInit.WAXED_CUT_PIGSTEEL.get(), BlockInit.CUT_PIGSTEEL.get());
        WAX_OFF_LIST.put(BlockInit.WAXED_INFECTED_CUT_PIGSTEEL.get(), BlockInit.INFECTED_CUT_PIGSTEEL.get());
        WAX_OFF_LIST.put(BlockInit.WAXED_CORRUPTED_CUT_PIGSTEEL.get(), BlockInit.CORRUPTED_CUT_PIGSTEEL.get());
        WAX_OFF_LIST.put(BlockInit.WAXED_ZOMBIFIED_CUT_PIGSTEEL.get(), BlockInit.ZOMBIFIED_CUT_PIGSTEEL.get());
        ...
        if (itemStack.getItem() instanceof AxeItem) {
            if (ZombifyingCutPigsteelBlock.PREVIOUS_BY_BLOCK.get().containsKey(blockState.getBlock())) {
                if (player instanceof ServerPlayer serverPlayer) {
                    CriteriaTriggers.ITEM_USED_ON_BLOCK.trigger(serverPlayer, pos, itemStack);
                }

                level.setBlock(pos, ZombifyingCutPigsteelBlock.PREVIOUS_BY_BLOCK.get().get(blockState.getBlock()).withPropertiesOf(blockState), 11);
